...
from dataset_jrdb_merged import collate_batch, batch_process_coords, get_datasets, create_dataset
...
def train(config, logger, experiment_name="", dataset_name=""):
    ...
    for epoch in range(config["TRAIN"]["epochs"]):
        ...
        for i in range(train_steps): 
            ...
            ################################
            start = time.time()
            # If you want no masking, pass None (or 0) and let your model create a default:
            loss, pred_joints = compute_loss(model, config, in_joints, out_joints, in_masks, out_masks, 0, epoch=epoch, mode='train', optimizer=None)
